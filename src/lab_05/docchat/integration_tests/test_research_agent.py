...
class TestResearchAgent(unittest.TestCase):
    ...
    def test_generate_with_azure_openai_documents(self):
        ...
        documents = [
            doc
            for doc in self.test_data.SAMPLE_DOCUMENTS
            if "Azure OpenAI" in doc.page_content
        ]
        ...
        print("✅ Azure OpenAI generation test passed")
    ...
    def test_generate_with_embedding_documents(self):
        ...
        documents = [
            doc
            for doc in self.test_data.SAMPLE_DOCUMENTS
            if "embedding" in doc.page_content.lower()
        ]
        ...
        print("✅ Embedding model generation test passed")
        print(
            f"   Answer mentions embedding concepts: {any(keyword in draft_answer for keyword in context_keywords)}"
        )
    ...
    def test_generate_with_machine_learning_documents(self):
        ...
        relevant_keywords_found = sum(
            1 for keyword in ml_keywords if keyword in draft_answer
        )

        self.assertGreater(relevant_keywords_found, 0)
        print("✅ Machine learning generation test passed")
        print(
            f"   Relevant keywords found: {relevant_keywords_found}/{len(ml_keywords)}"
        )
